	...
	// Counter num START
	  function isElementInViewport(el) {
		...
		return (
		  rect.top >= 0 &&
		  rect.left >= 0 &&
		  rect.bottom <= ($(window).height() || $(document.documentElement).height()) &&
		  rect.right <= ($(window).width() || $(document.documentElement).width())
		);
	  }
	
	  function handleScroll() {
		const targetElement = $(".s-numbers");
		if (isElementInViewport(targetElement[0])) {
		  targetElement.find(".numbers-num").each(function() {
			const targetCount = parseInt($(this).data("count"));
			const duration = 2000; // Измените время анимации по вашему усмотрению
			...
			const counter = setInterval(() => {
			  ...
			  $(this).text(Math.round(currentCount));
			}, frameDuration);
		  });
